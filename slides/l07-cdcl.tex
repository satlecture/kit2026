...
\subtitle{Lecture 7 \unimp{\normalfont -- Conflict-Driven Clause Learning}}
...
\begin{frame}{Overview}
    \begin{block}{Recap.}
        \textbf{Lecture 6: Classic Heuristics and Modern SAT Solving 1:}
...
\begin{blueblock}{Hot Paths in CDCL Solvers}
...
\begin{column}[t]{.3\linewidth}
    ...
    \doubleclin
    \rowcolor<5>{kit-green} \only<5->{2} & \only<5->{b} & \only<5->{\addr{2}} \\
    \cline{2-3}
    \end{tabularx}
\end{column}
\begin{column}[t]{.3\linewidth}
    ...
    $c$	& \addr{3} & \addr{1} & \\
    \doublecline
    \rowcolor<4->{kit-green} $\lnot c$ & \addr{2} & ~~~ & \\
    \cline{2-4}
    \end{tabularx}~\\[1em]
\end{column}
\begin{column}[t]{.3\linewidth}
...
\begin{frame}{Unit Propagation: Two Watched Literals}
    \begin{blueblock}{Motivation: Hot Path}
    \renewcommand{\arraystretch}{3}
    \begin{tabularx}{\linewidth}{l|c|L}
        \bf heat & $\varnothing~per~sec.$\footnote{Order of magnitude of average event count per second (in runs of Cadical on a large combined benchmark set)} & 
        \multirow[t]{3}{\linewidth}{%
\textbf{Idea:} Reduced occurrence tracking by only keeping the following invariant:\\[1ex]
Each yet unsatisfied clause is \highlo{watched by}, i.e., in the occurrence list of,\\
\highlo{two of its unassigned literals.}\\[1ex]
...
\begin{frame}{Unit Propagation: Two Watched Literals}
\vspace*{-1em}
\begin{blueblock}{Two Watched Literals: Optimizations}
\renewcommand{\arraystretch}{3}
\begin{tabularx}{\linewidth}{l|c|L}
    \bf heat & $\varnothing~per~sec.$\footnote{Order of magnitude of average event count per second (in runs of Cadical on a large combined benchmark set)} & 
...
\begin{itemize}\itemsep1em
    ...
    \item Further optimizations:\\[1ex]
    \begin{itemize}\itemsep1ex
        \item \textbf{Invariant:} the two first literals in each clause are the watched ones
        \item \textbf{Blocking literals:} keep a literal of each clause directly in occurrence list
    \end{itemize}
\end{itemize}
\end{block}
\begin{block}{Next Up}
...
\begin{frame}{Clause Forgetting}
\begin{blueblock}{Periodic Clause Forgetting: Heuristics}
\begin{itemize}\setlength{\itemsep}{1em}
    ...
    \item<2-> \textbf{Literal Block Distance (LBD)}\\[1ex]
    Keep clauses with a low number of decision levels%
    \only<2->\footnote{\href{https://www.ijcai.org/Proceedings/09/Papers/074.pdf}{Predicting Learnt Clauses Quality in Modern SAT Solvers, Audemard \& Simon (IJCAI 2009)}}\\[1em]
    \only<3>{\highlow{Why is low LBD good?}}
\end{itemize}
\end{blueblock}
\end{frame}
...
\begin{blueblock}{Take home: LBD correlates with number of touched communities}
\centering
...
\begin{blueblock}{Manage clauses differently in three tiers}
    \centering
    \begin{tabularx}{.95\linewidth}{l|l|X}
        \bf Tier & \bf Strategy & \bf Description\\
        \hline
        \texttt{core} & LBD & Permanently store clauses of LBD $\leq k$ (core-cut value, $3$ in practice)\\[5pt]
        \texttt{mid-tier} & LRU & Clauses stay here if used in recent conflicts\\[5pt]
        \texttt{local} & LRU & Keep fixed number of clauses (say $5000$) of highest activity
    \end{tabularx}
\end{blueblock}~\\
\begin{blueblock}{History}
    \begin{itemize}\itemsep1ex
    ...
    \item Note: The award-winning SAT solver \texttt{MapleCOMSPS (2016)} is a CoMinisatPS fork
    \end{itemize}
\end{blueblock}
\end{frame}
...
\begin{frame}{Instance from Termination Analysis, SAT}{
...
\begin{frame}{Instance from SV Competition, SAT}{
...
\begin{blueblock}{VSIDS Heuristic}
...
\begin{frame}{{\bf V}ariable {\bf S}tate {\bf I}ndependent {\bf D}ecaying {\bf S}um {\bf (VSIDS)}}
\begin{blueblock}{Common implementation: Binary Heap}
    \setcolsep{1em}
    \setrowsep{1.5}
...
\begin{frame}{{\bf V}ariable {\bf S}tate {\bf I}ndependent {\bf D}ecaying {\bf S}um {\bf (VSIDS)}}
\vspace*{-1ex}
\begin{blueblock}{Historic Implementations}~\\[-1em]
...
\end{minipage}
\end{blueblock}
\begin{exampleblock}{\textbf{Minisat (2003)}: Exponential VSIDS (EVSIDS)}
\textbf{Idea:} Exponential decay of scores $s(v)$ with damping factor $0 < f < 1$\\
$$
s'(v) := \begin{cases}
    f \cdot s(v) + (1 - f) & \text{if } v \text{ is to be bumped}\\
    f \cdot s(v) & \text{otherwise}
\end{cases}
$$\\[1ex]
\textbf{Theory:} Exponential Moving Average (EMA)\\[1ex]
\textbf{Implementation:} Score increment by $g^i$, with $i$ denoting the conflict-index and $g = \frac{1}{f}$ \quad (no decay)\\[2ex]
...
\begin{frame}{}{}
\href{https://doi.org/10.1007/978-3-319-24318-4_29}{Evaluating CDCL Variable Scoring Schemes (Biere \& Fröhlich, 2015)}\hfill\\[1em]
\centering
\includegraphics[width=.56\linewidth]{figures/l06/evsids-vmtf-acids-froehlich2015.png}
\end{frame}
    
\begin{frame}{Alternative and Hybrid Approaches}
\begin{blueblock}{Alternatives}
\textbf{Siege (2004)}: Variable Move To Front (VMTF)\\[1ex]
\textbf{HaifaSAT (2008)}: Clause Move To Front (CMTF)
\end{blueblock}~\\
\begin{blueblock}{Recent Hybrid Approaches} 
\begin{itemize}\setlength{\itemsep}{1em}
...
\begin{frame}{Modern Branching: Local Search Intergration}
\href{https://doi.org/10.1613/jair.1.13666 }{Better Decision Heuristics in CDCL through Local Search and Target Phases (Cai et al., 2022)}\\[1em]
\textbf{Branching:}\\
\begin{itemize}\itemsep1em
    \item Target Phases: cache and use the phases which led to the previously \highlo{largest} assignment
    \item Integrate Sprints of Local Search: use unit-propagation to \highlo{complete} the assignment (ignoring all conflicts)
    \item Rephasing: save the \highlo{best assignment found} during local search for phase selection (diversification)
\end{itemize}~\\
\textbf{Ordering:}\\
\begin{itemize}
    \item Import \highlo{Statistics}: use frequency of appearing in unsatisfied clauses to modify the variables VSIDS score
\end{itemize}
\end{frame}
