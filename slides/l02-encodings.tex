...
\begin{block}{Linear Time Algorithm for 2-SAT \href{https://doi.org/10.1016/0020-0190(79)90002-4}{(cf.~Aspvall et al., 1979)}}
	\begin{itemize}\setlength{\itemsep}{1em}
		...
		\item<3-> \highl{Find} Strongly Connected Components (SCC): 
			In an SCC, there is a path from every vertex to every other vertex.\\[1ex]
		$\rightarrow$ Tarjan's algorithm finds SCCs in $\mathcal{O}(|V|+|E|)$
		\item<4-> \highl{Check} for existence of complementary literals in the same SCC
	\end{itemize}
\end{block}~\\
\pause
