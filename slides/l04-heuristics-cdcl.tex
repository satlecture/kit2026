...
\begin{frame}{Overview}
	\begin{block}{Recap. Lecture 3: Elementary SAT Solving Algorithms}
	...
	\begin{block}{Today's Topics}
		\begin{itemize}\setlength{\itemsep}{1ex}
			\item Elementary SAT Solving Heuristics
			\begin{itemize}
...
\begin{itemize}\setlength{\itemsep}{1ex}
	...
	\item Gives easy sub-problems\\[3pt]
	\pause
	\begin{itemize}\setlength{\itemsep}{1ex}
...
\begin{column}{0.5\textwidth}
	\centering
		...
		\only<1>{Heavy-tailed distribution\\}
		\only<2>{\highlow{Heavy-tailed distribution}\\}
...
\begin{column}{0.5\textwidth}
	\centering
	\visible<2>{\includegraphics[width=\textwidth]{figures/l04/restart-inout-legend}}
\end{column}
...
\texttt{($u_1$, $v_1$) = (1, 1);}\\
\texttt{($u_{n+1}$, $v_{n+1}$) = $u_n$ \& -$u_n$ == $v_n$\ ?\ ($u_n$+1, 1) :\ ($u_n$, 2$v_n$);}~\\[1em]
\textbf{Example: }$(1,1), (2,1), (2,2), (3,1), (4,1), (4,2), (4,4), (5,1), \ldots$
\end{blueblock}
\end{frame}


\begin{frame}{Branching Polarity: Phase Saving}
Observation: Frequent {restarts} decrease performance on some satisfiable instances\\[1em]
\begin{blueblock}{Phase Saving / Assignment Caching}
Idea: Remember \highlow{last assignment of each variable} and use it first in branching\\[1ex]
\begin{columns}[T]
\begin{column}{0.7\textwidth}
\begin{itemize}\setlength{\itemsep}{1em}
	\item First implemented in \href{http://reasoning.cs.ucla.edu/rsat/}{RSAT (2006)}
	\item Result: Phase saving stabilizes positive effect of restarts
	\item Best results in combination with non-chronological backtracking (follows)
\end{itemize}
\end{column}
...
\end{tikzpicture}~\\
Example: $A$ and $B$ are satisfied,\\ searching in component $C$
\end{column}
\end{columns}
\end{blueblock}
\end{frame}
...
The edge labels denote \highlo{clauses}, node labels indicate a variable assignment and its \highlow{decision level}.\\
\begin{columns}[T]
\begin{column}{0.3\textwidth}
~\\[1ex]
\begin{itemize}\setlength{\itemsep}{1em}
...
\node (B) [circle, below=of A, yshift=1cm] { $_{\highlow 2}B=1$ }; 
\node (C) [circle, below=of B, yshift=1cm] { $_{\highlow 3}C=1$ };
\node (X) [circle, below=of C, yshift=1cm] { $_{\highlow 4}X=1$ };
\node (Y) [circle, right=of X, yshift=0.9cm] { $_{\highlow 4}Y=1$ };
\node (Z) [circle, right=of Y, yshift=0.9cm] { $_{\highlow 4}Z=1$ };
\node (empty) [circle, right=of Z] { \blitz };

\draw (A) -- (Y) node [midway, above] {\highlo 3 };
\draw (A) -- (Z) node [midway, below] {\highlo 5 };
\draw (A) -- (empty) node [midway, above] {\highlo 7 };
\draw (Y) -- (Z) node [midway, above] {\highlo 5 };
\draw (Z) -- (empty) node [midway, above] {\highlo 7 };
\draw (X) -- (Y) node [midway, above] {\highlo 3 };
\draw (Y) -- (empty) node [near start, below] {\highlo 7 };
\end{tikzpicture}%
\end{column}%
...
Implement trail as \highlo{stack of literals} together with a pointer to the \highlo{reason clause} (\emph{null} for decisions) and the \highlo{decision level}. On each conflict, use the trail to trace back the implications to the conflict sources.\\[1em]
\textbf{Example:} Trail with conflicting clause $\{\lnot A, \lnot Y, \lnot Z \}$\\[1ex]
\begin{columns}[T]
\begin{column}{.3\linewidth}
	\begin{tabular}{ccl}
	\textbf{Var.} & \textbf{Lvl.} & \textbf{Reason} \\
...
\end{itemize}
\item In 1-UIP learning, the backtrack level is always the second highest level in a conflict clause.
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep1em
	\item \href{https://doi.org/10.1007/978-3-030-51825-7_3}{Clause Size Reduction with all-UIP Learning (Feng and Bacchus, 2020)}
	\item \href{https://doi.org/10.1007/978-3-030-80223-3\_12}{Efficient All-UIP Learned Clause Minimization (Fleury and Biere, 2021)}
\end{itemize}
\end{frame}

			
\begin{frame}{Resolution Proof}
\begin{blueblock}{Properties of conflict clause C}
\begin{itemize}\setlength{\itemsep}{1em}
	\item $F \models C$ 
	\item $F \cup \lnot C \vdash_{UP} \bot$
	\item $\forall D \subseteq C, D \notin F$
\end{itemize}
\end{blueblock}~\\
\begin{blueblock}{Certificates for Unsatisfiability}
\begin{itemize}\setlength{\itemsep}{1em}
	\item sequence of learned clauses serves as a \highlo{proof of unsatisfiability}
	\item can be used to validate the correctness of the SAT result in \highlo{high risk applications}
\end{itemize}
\end{blueblock}
\end{frame}
