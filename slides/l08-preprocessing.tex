...
~\\[1em]
\textbf{Lecture 6: \highl{Modern SAT Solving 1}}
\begin{itemize}%\setlength{\itemsep}{1em}
    ...
    \item Restarts
\end{itemize}~\\
\textbf{Lecture 7: \highlo{Modern SAT Solving 2}}
\begin{itemize}%\setlength{\itemsep}{1em}
...
\end{itemize}~\\
\visible<2->{
    \textbf{Today: \highlow{Preprocessing}}
}
\end{column}
...
\begin{algorithm}[H]
    ...
    \SetKwFunction{branching}{\highlo{BRANCHING}}
    ...
    \SetKwFunction{preprocessing}{\only<1>{PREPROCESSING}\only<2->{\highlow{PREPROCESSING}}}
    ...
    \While {A is not complete} {
        ...
        \If {A falsifies a clause in $F$} {
            ...
            \Else {
                ...
                add clause to $F$ and \highl{backtrack to level}\;
                \textbf{continue}\;
            }
        }
        \lIf {\restart} {
...
Select clause $C$ and check \highlo{if it is subsumed} by any other clause $D \subseteq C$.~\\[1ex]
...
\highlow{How to do that efficiently?}}
~\\[1ex]
\visible<4>{
\begin{itemize}\setlength{\itemsep}{1ex}
...
\begin{column}{.55\linewidth}~\\
    Select clause $D$ and check \highlo{if it subsumes} any other clause $C \supseteq D$.\\[1ex]
    ...
    \begin{itemize}\setlength{\itemsep}{1ex}
        ...
        \item<4> \textbf{Optimization 2:} Use signatures to skip the majority of subsumption checks (cf. Bloom filters)
    \end{itemize}
    }
\end{column}
...
\item What does BCE do to $F = \bigl\{ \{ q \}, \{ \lnot q, o, p \}, \{ \lnot q, \lnot o, \lnot p \} \bigr\} \cup \bigl\{ \{ p, r \}, \{ \lnot p, s \} \bigr\} \cup E$?
\end{itemize}
\end{frame}
