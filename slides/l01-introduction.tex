...
\column{\kitthreecolumns}	
	...
	\end{align*}
	\end{example}~\\
	\only<4>{\highlow{Is this instance satisfiable?}}
\end{columns}
\end{frame}
...
\begin{itemize}\itemsep1ex
	\item<2-> We do not have a polynomial algorithm for SAT (yet) ~\cChangey{0}
	\item<3-> If $P \neq NP$ then we will never have a polynomial algorithm for SAT ~\cChangey{-1}
	\item<4-> All the known algorithms for SAT have exponential runtime in the \textbf{worst case} ~\cChangey{-2}
\end{itemize}
\end{block}~\\
\only<5>{
