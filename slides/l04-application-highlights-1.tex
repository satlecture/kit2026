...
\subtitle{Lecture 4 \unimp{\normalfont -- Application Highlights}}
...
\begin{frame}{Overview}
	\begin{block}{Recap Lecture 3}
		\begin{itemize}
			\item Stochastic local search
			\item Resolution, saturation algorithm
			\item From Davis Putnam to DPLL
		\end{itemize}
	\end{block}
...
\begin{frame}{Why consider the Application View? (2/2)}
	...
	\vspace*{6mm}
	%\centering
	\hspace*{2cm}\includegraphics[width=0.5\textwidth]{figures/l10/algorithm-engineering.pdf}
\end{frame}
		...
		\uncover<2->{
			\begin{block}{Miter Formula}
				\begin{itemize}
					...
					\item Satisfiable $\Leftrightarrow$ ($F \not\equiv G$)\quad\quad(\highlo{why this way?})\ \ 
				\end{itemize}
			\end{block}	
		}
	\end{minipage}%
	...
	\begin{minipage}[c][12cm][t]{0.57\textwidth}
		...
		\begin{itemize}
			...
			\uncover<1->{%
				...
				\begin{itemize}
					\item Circuits are \highlo{not equivalent}~~(``\highlo{buggy miter}'')
...
\begin{frame}{CEC Techniques~\cite{weaver2015equivalence}}
	\begin{minipage}[c][8cm][t]{0.5\textwidth}
		\textbf{Improving CEC}: Try to \highl{merge equivalent sub-circuits}
		\begin{itemize}
...
\begin{frame}{Our Latest Work on CEC}
	\vspace*{-8mm}
	\textbf{Parallel~~SAT~~Swee\text{\tiny \textcolor{white}{xa}}\!\!\!ping}~~~\unimp{(Rigi-Luperti, Biere, Schreiber 2026; to be published)}
	\begin{itemize}
		\item No information on circuits -- operates \highl{purely on CNF}!
		\item Parallelization of Kissat's sweeping:\\
		Sweeps limited environments in the \highl{Variable Incidence Graph} starting from selected variables
	\end{itemize}
	%\\[3mm]
	...
	\vspace*{5mm}
	\centering
	\includegraphics[scale=2]{figures/l11/parallel-sat-sweeping.pdf}
	
	\vspace*{3mm}
	\centering
	\unimp{\small Four workers}
	}
	
	\only<2->{
	\includegraphics[scale=1.5]{figures/l11/coupled-cdf-hw20-maxx300.pdf}
	\includegraphics[scale=1.5]{figures/l11/sweep-cdf-hw12-maxx300.pdf}
	\includegraphics[scale=1.5]{figures/l11/weakscaling-hw12.pdf}
	
	\vspace*{1mm}
	\centering
	\unimp{\small Benchmarks from Hardware Model Checking Competition (HWMCC) 2012 and 2020}
	}
\end{frame}
...
\begin{frame}{Is SAT-based MAPF worthwhile?}
	...
	\begin{minipage}[c][8cm][t]{0.45\textwidth}
		...
		\includegraphics[height=8cm]{figures/l11/mapf-large.pdf}
	\end{minipage}%
	...
	\begin{minipage}[c][8cm][t]{0.45\textwidth}
		...
		\includegraphics[height=8cm]{figures/l11/mapf-small.pdf}
	\end{minipage}%
\end{frame}
		...
		\only<2>{
			\begin{block}{SAT-based approach~\cite{narodytska2018learning}}
				\begin{itemize}
					...
					\item Constrain that \highl{0-leaves exclude all 1-labeled samples}\\
					and vice versa
...
\begin{frame}{More on SAT Solving $\times$ Machine Learning}
	...
	\begin{itemize}
		...
		\unimp{\small{}Fuchs, Tobias, Jakob Bach, and Ashlin Iser. ``Active Learning for SAT Solver Benchmarking.'' TACAS 2023.}
	\end{itemize}
\end{frame}
...
\begin{frame}{Application Highlights: Takeaways}
	\vspace*{-7mm}
	\begin{itemize}\setlength{\itemsep}{1ex}
		\item SAT is an \highl{\textbf{essential and well-established tool}} particularly for
		\begin{itemize}
			\item software \& hardware verification
			\item electronic design automation
			\item security and cryotography
		\end{itemize}
		...
		\begin{itemize}
			...
			\item More often than not, \highl{incremental} and \highl{iterative} approaches are needed
		\end{itemize}
		%\item \textbf{Many more}, sometimes important, \textbf{applications} of SAT we didn't talk about
		%\begin{itemize}
		%	\item Test pattern generation, graph theory, theorem proving, social choice theory, puzzle solving, knowledge compilation, \ldots
		%\end{itemize}
	\end{itemize}
\end{frame}
