...
\begin{block}{What can happen after assigning $P := \{ a_1, \ldots, a_{i \leq k}\}$ and propagating $\overline P$?}
\begin{itemize}\setlength{\itemsep}{1em}
    ...
    \begin{itemize}\normalsize\setlength{\itemsep}{5pt}
        ...
        \item[] \only<5>{\highlow{Why is that sound?}}\onslide<6->{$F \setminus C \models \overline P \rightarrow a$, so $P \cup \{a\}$ already \emph{subsumes} $C$.}
    \end{itemize}
    ...
    \begin{itemize}\normalsize\setlength{\itemsep}{5pt}
        ...
        \item[] \only<8>{\highlow{Why is that sound?}}\onslide<9->{$\overline P \subseteq \overline{C \setminus \{a\}}$ gives $F \setminus C \land \overline{C \setminus \{a\}} \vdash_{\mathit{UP}} \lnot a$, the ALE condition $\Rightarrow F \setminus C \models C \setminus \{a\}$.}
    \end{itemize}
\end{itemize}
\end{block}
\end{frame}
