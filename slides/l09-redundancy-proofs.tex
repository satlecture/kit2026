...
If $F \setminus C \land \overline C \vdash_{\mathop{UP}} \bot$, then $F \setminus C \models C$.\\[1ex]
...
\vspace*{-1em}
Given clause $C = \{ a_1, a_2, \ldots, a_k \}$. Sequentially assign $\overline a_1, \overline a_2, \ldots$ and propagate over $F \setminus \{C\}$.\\[1ex]
\begin{block}{What can happen after assigning and propagating $\overline P$ with $P := \{ a_1, \ldots, a_{i \leq k}\}$?}
\begin{itemize}\setlength{\itemsep}{1em}
    ...
    \begin{itemize}\normalsize\setlength{\itemsep}{5pt}
        ...
        \item[] \only<2>{\highlow{Why is that sound?}}\onslide<3->{$\overline P \subseteq \overline{C}$ gives $F \setminus C \land \overline{C} \vdash_{\mathit{UP}} \bot$, the ATE condition $\Rightarrow F \setminus C \models C$.}
    \end{itemize}
    \item \highlo{for some $a \in C \setminus P, a$ is propagated true}: $F \setminus C \land \overline P \vdash_{\mathit{UP}} a$.
    \begin{itemize}\normalsize\setlength{\itemsep}{5pt}
        \item[] \only<4>{\highlow{What can we do in this case?}}\onslide<5->{$\Rightarrow$ \textbf{Clause elimination:} remove $C$.}
        \item[] \only<5>{\highlow{Why is that sound?}}\onslide<6->{The extended prefix $\overline{P} \cup \{\overline a\}$ leads to a conflict, so $C$ is redundant (the ATE condition, just like above).}
    \end{itemize}
    \item \highlo{for some $a \in C \setminus P, a$ is propagated false}: $a$ can never help satisfy $C$ in this context.
    \begin{itemize}\normalsize\setlength{\itemsep}{5pt}
        \item[] \only<7>{\highlow{What can we do in this case?}}\onslide<8->{$\Rightarrow$ \textbf{Literal elimination:} remove $a$ from $C$ and continue.}
        \item[] \only<8>{\highlow{Why is that sound?}}\onslide<9->{$\overline P \subseteq \overline{C \setminus \{a\}}$ gives $F \setminus C \land \overline{C \setminus \{a\}} \vdash_{\mathit{UP}} \overline a$, the ALE condition $\Rightarrow F \setminus C \models C \setminus \{a\}$.}
    \end{itemize}
\end{itemize}
\end{block}
\end{frame}
